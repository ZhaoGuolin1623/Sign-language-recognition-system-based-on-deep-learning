...
class CSL_Continuous_Char(Dataset):
    def __init__(self, data_path, corpus_path, frames=128, train=True, transform=None):
        ...
        try:
            obs_path = [os.path.join(self.data_path, item) for item in os.listdir(self.data_path)]
            # obs_path = [os.path.join(self.data_path, item) for item in os.listdir(self.data_path) if not item.endswith('.txt')]# https://blog.csdn.net/weixin_34659856/article/details/113508398
            # obs_path = [os.path.join(self.data_path) for item in os.listdir(self.data_path) if not item.endswith('.txt')]
            self.data_folder = sorted([item for item in obs_path if os.path.isdir(item)])
        except Exception as e:
            raise
